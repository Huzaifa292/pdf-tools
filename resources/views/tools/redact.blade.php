<x-app-layout>
<x-slot name="title">Redact PDF - PDFTools</x-slot>
<div class="max-w-2xl mx-auto py-10">
    <div class="text-center mb-8">
        <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-gray-600 to-slate-500 flex items-center justify-center text-4xl mx-auto mb-5 shadow-lg">⬛</div>
        <h1 class="text-3xl font-extrabold text-gray-900">Redact PDF</h1>
        <p class="text-gray-500 mt-2">Permanently remove sensitive text from your PDF.</p>
    </div>
    @if(session('error'))
    <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">⚠️ {{ session('error') }}</div>
    @endif
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
        <form method="POST" action="/redact-pdf" enctype="multipart/form-data" id="upload-form">
            @csrf
            ...
            <div class="mt-4">
                <label class="block text-sm font-semibold text-gray-700 mb-1">Text to Redact</label>
                ...
                <p class="text-xs text-gray-400 mt-1">Separate multiple words with comma</p>
            </div>
