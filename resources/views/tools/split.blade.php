<x-app-layout>
<x-slot name="title">Split PDF - PDFTools</x-slot>
<div class="max-w-2xl mx-auto text-center py-10">
    <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center text-4xl mx-auto mb-6 shadow-xl">✂️</div>
    <h1 class="text-4xl font-extrabold text-white">Split PDF</h1>
    <p class="text-slate-400 mt-3 text-lg">Separate pages into individual PDF files.</p>
    ...
    <form method="POST" action="/split-pdf" enctype="multipart/form-data" class="mt-8">
        @csrf
        ...
        <div class="mt-4 text-left">
            <label class="text-slate-300 text-sm font-medium">Pages to extract (e.g. 1,3,5-8) — leave blank for all</label>
            <input type="text" name="pages" placeholder="1,3,5-8"
        ...
        <button type="submit" class="mt-6 w-full py-4 bg-red-500 hover:bg-red-600 rounded-2xl font-bold text-white text-lg transition">Split PDF ↓</button>
    </form>
