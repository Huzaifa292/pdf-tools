<x-app-layout>
<x-slot name="title">PDF to JPG - PDFTools</x-slot>
<div class="max-w-2xl mx-auto py-10">
    <div class="text-center mb-8">
        <div class="w-20 h-20 rounded-3xl bg-gradient-to-br from-rose-500 to-pink-500 flex items-center justify-center text-4xl mx-auto mb-5 shadow-lg">📸</div>
        <h1 class="text-3xl font-extrabold text-gray-900">PDF to JPG</h1>
        <p class="text-gray-500 mt-2">Convert each PDF page into a JPG image.</p>
    </div>
    @if(session('error'))
    <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">⚠️ {{ session('error') }}</div>
    @endif
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
        <form method="POST" action="/pdf-to-jpg" enctype="multipart/form-data" id="upload-form">
            @csrf
